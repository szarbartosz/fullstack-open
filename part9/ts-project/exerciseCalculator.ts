...
const parseTrainingArguments = (args: Array<string>): TrainingDaysParameters => {
  if (args.length < 4) throw new Error('Not enough arguments');

  let correctDaysData: boolean = true;
...
const calculateExcercises = (training: number[], target: number): void => {
...
try {
  const { training, target } = parseTrainingArguments(process.argv)
  calculateExcercises(training, target)
} catch (e) {
